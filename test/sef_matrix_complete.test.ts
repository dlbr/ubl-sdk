...
describe('🛡️ SEF Matrix XML Builder — Kompletan Poreski i XML Audit', () => {
  ...
  it('1. Avansna Faktura (386) sa obračunatim PDV-om', () => {
    const xml = SefUblBuilder.buildAvansni({
      broj: 'AV-1', 
      pibProdavca: '1000000012345', 
      pibKupca: '2000000023456', 
      osnovica: 1000, 
      pdv: 200,
      datumUplate: '2026-05-26',   
      referentniRacun: 'PONUDA-1'
    });
    
    expect(xml).toContain('<cbc:CustomizationID>urn:cen.eu:en16931:2017#compliant#urn:mfin.gov.rs:srbdt:2.1</cbc:CustomizationID>');
  ...
  it('2. Konačna faktura sa zatvaranjem avansa (380)', () => {
    const xml = SefUblBuilder.buildKonacniSaAvansom({
      ...
      pibKupca: 'RS200000002',
      avansBroj: 'AV-1', 
      avansDatum: '2026-05-25',
      ukupnaOsnovica: 2000, 
      ukupniPdv: 400, 
      odbitakAvansaSaPdv: 600,
      pdvStopa: 20, 
  ...
  describe('📊 v3.8.0 Master Specifikacija Alignment Audit', () => {
    ...
    it('1. Poreska kategorija OE (Zamenjeno sa N) sa POZITIVNIM smerom', () => {
      const xml = SefUblBuilder.buildSmanjenje({
        broj: 'STR-N-POS', 
        pibProdavca: '100000001', 
        pibKupca: '100000002',
        referentniRacun: 'INV-1', 
        razlog: 'Specifičan režim',
        iznosZaSmanjenjeOsnovice: 1000, 
        iznosZaSmanjenjePdv: 0,
        poreskaKategorija: 'OE',
        smerDokumenta: 'POZITIVAN'
      });
      
      expect(xml).toContain('>OE</cbc:ID>'); 
      expect(xml).toContain('>20.00</cbc:Percent>');
    ...
    it('2. Poreska kategorija OE (Zamenjeno sa N) sa NEGATIVNIM smerom', () => {
      const xml = SefUblBuilder.buildSmanjenje({
        broj: 'STR-N-NEG', 
        pibProdavca: '100000001', 
        pibKupca: '100000002',
